...
def test_canonical_request_path_encodes_decoded_spaces() -> None:
    assert (
        canonical_request_path("/s3/pithosys/Local Testing/file.csv")
        == "/s3/pithosys/Local%20Testing/file.csv"
    )
...
def test_canonical_request_path_does_not_double_encode_percent_twenty() -> None:
    ...
    assert (
        canonical_request_path("/s3/pithosys/Local%20Testing/file.csv")
        == "/s3/pithosys/Local%20Testing/file.csv"
    )
...
def test_canonical_request_path_preserves_slashes() -> None:
    assert (
        canonical_request_path("/s3/pithosys/photos/2024/cat.jpg")
        == "/s3/pithosys/photos/2024/cat.jpg"
    )
