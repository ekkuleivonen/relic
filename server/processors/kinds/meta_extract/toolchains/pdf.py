"""PDF parser. Writes compact discovery metadata into file meta."""
...
def empty_pdf_meta(*, existing_meta: dict[str, Any] | None = None) -> dict[str, Any]:
    """Shape matching parse() output for failed or unavailable PDF parsing."""
...
def parse_pdf(*, content: bytes, existing_meta: dict[str, Any] | None = None) -> dict[str, Any]:
    """Parse PDF bytes for storage in file meta. Never raises."""
    if not content:
        return empty_pdf_meta(existing_meta=existing_meta)
    try:
...
def parse(content: bytes, *, existing_meta: dict[str, Any] | None = None) -> dict[str, Any]:
    """Parse PDF bytes into the common discovery meta dict."""
