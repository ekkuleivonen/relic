"""HTML parser. Returns a discovery payload for the html_meta section."""
...
def empty_html_meta() -> dict[str, Any]:
    return empty_parser_discovery(tags=["html", "webpage"])


def parse_html(
    *, content: bytes, file_info: dict[str, Any] | None = None
) -> dict[str, Any]:
    """Parse HTML bytes into a discovery payload. Never raises."""
    if not content:
        return empty_html_meta()
    try:
...
def parse(
    ...
    return _build_discovery(details=details, file_info=file_info)
...
def _filename_terms(file_info: dict[str, Any] | None) -> list[str]:
    fn = ""
    if file_info:
        fn = str(file_info.get("original_filename") or "")
