...
def empty_audio_meta(*, file_info: dict[str, Any] | None = None) -> dict[str, Any]:
...
def parse_audio(*, content: bytes, file_info: dict[str, Any] | None = None) -> dict[str, Any]:
    """Parse audio bytes for storage in file meta. Never raises."""
    if not content:
        return empty_audio_meta(file_info=file_info)
    try:
...
def parse(content: bytes, *, file_info: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    details = _extract_details(audio=audio, file_info=file_info)
    return _build_discovery_meta(details=details, file_info=file_info)


def _extract_details(*, audio: Any, file_info: dict[str, Any] | None) -> dict[str, Any]:
    info = getattr(audio, "info", None)
    tags = getattr(audio, "tags", None)
    container = _container(audio=audio, file_info=file_info)
...
def _container(*, audio: Any, file_info: dict[str, Any] | None) -> str | None:
