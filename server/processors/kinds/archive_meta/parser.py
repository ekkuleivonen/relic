...
def empty_archive_meta(*, file_info: dict[str, Any] | None = None) -> dict[str, Any]:
...
def parse_archive(
    *, content: bytes, file_info: dict[str, Any] | None = None
    ...
    if not content:
        return empty_archive_meta(file_info=file_info)
    try:
...
def parse(content: bytes, *, file_info: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    kind = _archive_kind(content=content, file_info=file_info)
    if kind == "zip":
...
def _archive_kind(*, content: bytes, file_info: dict[str, Any] | None) -> str | None:
    filename = _original_filename(file_info).lower()
