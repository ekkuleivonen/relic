"""CSV parser. Returns a discovery payload for the csv_meta section.
...
def empty_csv_meta() -> dict[str, Any]:
    return empty_parser_discovery(tags=["data"])


def parse_csv(*, content: bytes) -> dict[str, Any]:
    """Parse CSV bytes into a discovery payload. Swallows unexpected errors."""
    try:
...
def parse(content: bytes | IO[bytes]) -> dict[str, Any]:
    """Parse CSV bytes (or stream) into the common discovery payload."""
