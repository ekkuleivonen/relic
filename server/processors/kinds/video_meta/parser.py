...
def empty_video_meta(*, file_info: dict[str, Any] | None = None) -> dict[str, Any]:
...
def parse_video(*, content: bytes, file_info: dict[str, Any] | None = None) -> dict[str, Any]:
    """Parse video bytes for storage in file meta. Never raises."""
    if not content:
        return empty_video_meta(file_info=file_info)
    try:
...
def parse(content: bytes, *, file_info: dict[str, Any] | None = None) -> dict[str, Any]:
    """Parse supported video bytes into the common discovery meta dict."""
    details = _parse_mp4(content, file_info=file_info)
    if not details["container"]:
        details = _parse_avi(content, file_info=file_info)
    if not details["container"]:
        raise ValueError("Unsupported video container")
    return _build_discovery_meta(details=details, file_info=file_info)


def _parse_mp4(content: bytes, *, file_info: dict[str, Any] | None) -> dict[str, Any]:
    details: dict[str, Any] = {
        ...
        "has_video": False,
        "codec": None,
        "filename_keywords": _filename_keywords(_original_filename(file_info)),
    }

    if not _has_mp4_signature(content):
        return details

    details["container"] = _container_from_meta(file_info)
...
def _parse_avi(content: bytes, *, file_info: dict[str, Any] | None) -> dict[str, Any]:
    details: dict[str, Any] = {
        "container": None,
        "duration_seconds": None,
        "width": None,
...
def _container_from_meta(file_info: dict[str, Any] | None) -> str | None:
