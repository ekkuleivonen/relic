...
def empty_office_doc_meta(*, file_info: dict[str, Any] | None = None) -> dict[str, Any]:
...
def parse_office_doc(
    *, content: bytes, file_info: dict[str, Any] | None = None
    ...
    if not content:
        return empty_office_doc_meta(file_info=file_info)
    try:
...
def _build_discovery_meta(
    *, details: dict[str, Any], file_info: dict[str, Any] | None
    ...
    type_tags = _type_tags(details=details, file_info=file_info)
...
def _type_tags(*, details: dict[str, Any], file_info: dict[str, Any] | None) -> list[str]:
    observed = " ".join(
        value
        for value in [
            _original_filename(file_info),
            details["title"],
            details["subject"],
            *details["headings"],
            details["text"][:500],
