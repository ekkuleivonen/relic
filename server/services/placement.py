...
from models import Blob, Bucket, BucketProbe, Folder
...
def hotness_for_buckets(
    ...
    *,
    ...
    grouped: dict[uuid.UUID, list[tuple[int | None, ...]]] = defaultdict(list)
    for bucket_id, _observed_at, *latencies in rows:
        if len(grouped[bucket_id]) >= effective_window:
            continue
        grouped[bucket_id].append(tuple(latencies))
    ...
    for bucket_id, samples in grouped.items():
        ...
        for op_index, _op_name in enumerate(_PROBE_OPS):
            values = [
                ...
                if sample[op_index] is not None
            ]
            if values:
        ...
        if averaged:
            by_id[bucket_id] = BucketHotness(
                bucket=bucket,
                avg_latency_ms=sum(averaged) / len(averaged),
                reachable=True,
